...
const CameraSelect = ({ videoTrack, style = {} }) => {
  ...
  useEffect(() => {
    if (videoTrack) {
      ...
      AgoraRTC.getCameras().then(cams => {
        setItems(cams.map(item => ({ label: item.label, key: item.deviceId })))
      })
    }
  }, [videoTrack])
  ...
  useEffect(() => {
    AgoraRTC.onCameraChanged = async changedDevice => {
      ...
      setItems(cameras.map(item => ({ label: item.label, key: item.deviceId })))
