...
const advancedItems = [
  ...
  {
    title: 'imageenhance',
    content: 'imageenhanceNote',
    to: "/video-beauty-effect"
  },
  {
    title: 'audioeffect',
...
const IndexPage = () => {
  ...
  return <div className='index-page'>
    {finalItems.map(({ items, title }, index) =>
      <div key={title} style={{ marginTop: index !== 0 ? "15px" : 0 }}>
        <Title>{t(title)}</Title>
        <Row gutter={[16, 16]}>
          {items.map((item, index) => <Col span={8} key={index}>
            <Card title={t(item.title)}>
              ...
              <div className='card-footer'>
                {
                  item.to ? <Link to={item.to} state={{ title: item.title }}>
