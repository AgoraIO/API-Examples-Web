...
function BasicMute() {
  ...
  */
  ...
  const muteAudio = async () => {
    await audioTrack.setMuted(!audioTrackMuted);
    setAudioTrackMuted(!audioTrackMuted);
  };
  const muteVideo = async () => {
    await videoTrack.setMuted(!videoTrackMuted);
  ...
  return <div className="padding-20">
    ...
    <Space style={{
      ...
      <Button type="primary" onClick={muteAudio} disabled={!joined}>{audioTrackMuted ? "Unmute Audio" : "Mute Audio"}</Button>
      <Button type="primary" onClick={muteVideo} disabled={!joined}>{videoTrackMuted ? "Unmute Video" : "Mute Video"}</Button>
    </Space>
