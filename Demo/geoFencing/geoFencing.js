...
$(() => {
  initAreas();
  $(".areas-list").delegate("a", "click", function (e) {
...
function initAreas() {
  areas.forEach(profile => {
    $(".areas-list").append(`<a class="dropdown-item" label="${profile.label}" href="#">${profile.label}: ${profile.detail}</a>`);
  });
  area = areas[0];
  $(".areas-input").val(`${area.detail}`);
}