...
$(async () => {
  ...
  $('#closeDeviceTestModel').click(function (e) {
    closeLocalTracks();
  });
...
async function doNetworkTest() {
  ...
  downlinkClient.on("network-quality", (quality) => {
    ...
    $("#remote-audio-stats").JSONView(
      JSON.stringify(downlinkClient.getRemoteAudioStats()[upClientUid]),
    );
    console.log("downlink video stats", downlinkClient.getRemoteVideoStats()[upClientUid]);
    $("#remote-video-stats").JSONView(
      JSON.stringify(downlinkClient.getRemoteVideoStats()[upClientUid]),
    );
  });
}
