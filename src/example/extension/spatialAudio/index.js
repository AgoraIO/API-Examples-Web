...
var remoteUsers = [];
var remoteUsersSound = ["./resources/3.mp3"];
var localPlayerSound = ["./resources/2.mp3"];
var localPlayTracks = [];
  ...
  for (let i = 0; i < localPlayerSound.length; i++) {
    setTimeout(async () => {
      try {
        ...
        console.error(
          `localPlayerSound[${i}] with buffersource track ${localPlayerSound[i]} play fail: ${error}`,
        );
      }
    }, 500 * i);
  }
  ...
  for (let i = 0; i < remoteUsersSound.length; i++) {
    setTimeout(async () => {
      try {
        ...
        console.error(
          `remoteUsersSound[${i}] with buffersource track ${remoteUsersSound[i]} join and publish fail: ${error}`,
        );
      }
    }, 500 * i);
  }
...
async function mockRemoteUserLeave() {
  for (let i = 0; i < remoteUsersSound.length; i++) {
    try {
      ...
      console.error(
        `speaker[${i}] with buffersource track ${remoteUsersSound[i]} leave fail: ${error}`,
      );
    }
  }
}
