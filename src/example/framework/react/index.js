...
$(function () {
  ...
  const MicSelect = function (props) {
    ...
    return <select class="form-select mic-list" value={curValue} onChange={onChange}>
  ...
  const CamSelect = function (props) {
    ...
    AgoraRTC.onCameraChanged = useCallback(async changedDevice => {
    ...
    return <select class="form-select cam-list" value={curValue} onChange={onChange}>
      {items.map(item => <option value={item.value}>{item.label}</option>)}
    </select>
  }
  ...
  const StreamPlayer = function (props) {
    ...
    return <div class="player" ref={videoRef}>
    ...
    return <>
      ...
      <div class="video-group col-md-12 col-lg-6">
        ...
        <section class="card">
          ...
          <div class="card-body">
            <StreamPlayer videoTrack={videoTrack} uid={options.uid} options={{
          ...
          <div class="card-body">
            <div id="remote-playerlist">{
              Object.keys(remoteUsers).map(uid => {
                ...
                return <StreamPlayer key={uid} videoTrack={user.videoTrack} audioTrack={user.audioTrack} uid={uid}></StreamPlayer>
              })
            }</div>
          </div>
        </section>
        ...
        <form id="join-form" name="join-form">
          <div class="join-info-group">
            <div class="form-item">
              ...
              <input class="form-control" id="channel" type="text" placeholder="enter channel name"
                value={options.channel} onChange={(e) => setOptions(pre =>
                ({
                  ...pre,
            ...
            <div class="form-item">
              ...
              <input class="form-control" id="uid" type="text" value={options.uid}
                onChange={(e) => setOptions(pre => ({
                  ...pre,
                  uid: e.target.value
                }))}
                onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
                onafterpaste="this.value=this.value.replace(/[^0-9]/g,'')" placeholder="Enter the user ID"></input>
            </div>
          </div>
          <div class="button-group">
            ...
            <section class="step">
              ...
              <button type="button" id="step-create" class="btn btn-primary btn-sm" disabled={createDisabled} onClick={stepCreate}>
                Create Client
              </button>
            </section>
            {/* <!-- Step2 --> */}
            <section class="step">
              ...
              <button type="button" id="step-join" class="btn btn-primary btn-sm" disabled={joinDisabled} onClick={stepJoin}>
                Join Channel</button>
            </section>
            {/* <!-- Step3 --> */}
            <section class="step">
              ...
              <div class="form-check">
                ...
                <input class="form-check-input" type="checkbox" value="" id="mirror-check"
                  checked={mirrorChecked} disabled={mirrorCheckDisabled} onChange={e => setMirrorChecked(e.target.value)}></input>
              </div>
              <button type="button" id="step-publish" class="btn btn-primary btn-sm" disabled={publishDisabled} onClick={stepPublish}>
            ...
            <section class="step">
              ...
              <input class="form-control" type="text" value={remoteUid} onChange={e => setRemoteUid(e.target.value)} id="remote-uid" placeholder="Please input remote userId"
                style={{ maxWidth: "300px" }}></input>
              <div class="mt-2 mb-1">
                <span class="form-check audio-check">
                  <input class="form-check-input" type="checkbox" value="" id="audio-check"
                ...
                <span class="form-check video-check">
                  <input class="form-check-input" type="checkbox" value="" id="video-check"
              ...
              <button type="button" id="step-subscribe" class="btn btn-primary btn-sm" disabled={subscribeDisabled} onClick={stepSubscribe}>Subscribe & Play</button>
            </section>
            {/* <!-- Step5 --> */}
            <section class="step">
              <label class="form-label step-title"><span>Step5</span> Leave Channel</label>
              <button type="button" id="step-leave" class="btn btn-danger btn-sm" disabled={leaveDisabled} onClick={stepLeave}>
